...
import json
#这是一个用于将字符串转换为布尔值的函数。它的参数是一个字符串 s，函数会先检查这个字符串是不是 'False' 或 'True' 中的一个，如果不是则抛出 ValueError 异常。如果是，那么函数会返回 True 或 False，具体取决于字符串是否为 'True'。如果字符串是 'True'，则返回 True，否则返回 False。这个函数通常用于在读取配置文件或命令行参数时将字符串转换为布尔值，以便程序可以正确地处理它们。
def str2bool(s):
...
parser.add_argument('--threshold_item', default=0.96, type=float)
...
sampler = WarpSampler(user_train, usernum, itemnum, batch_size=args.batch_size, maxlen=args.maxlen, threshold_user=args.threshold_user,
            threshold_item=args.threshold_item,n_workers=3)
model = Model(usernum, itemnum, args)
#sess.run(tf.global_variables_initializer())

aug_data_signature = './aug_data/{}/lr_{}_maxlen_{}_nblocks_{}_drate_{}_l2_{}_nheads_{}_gen_num_{}_M_{}'.format(args.dataset, args.lr, args.maxlen,  args.num_blocks, args.dropout_rate, args.l2_emb, args.num_heads, args.reversed_gen_number, args.M)
print(aug_data_signature)

model_signature = 'lr_{}_maxlen_{}_nblocks_{}_drate_{}_l2_{}_nheads_{}_gen_num_{}'.format(args.lr, args.maxlen,  args.num_blocks, args.dropout_rate, args.l2_emb, args.num_heads, 5)
...
try:
    for epoch in range(1, args.num_epochs + 1):
        ...
        if (epoch % 20 == 0 and epoch >= 200) or epoch == args.num_epochs:
            ...
            T += t1
            print("start testing")
            RANK_RESULTS_DIR = "./rank_results/{args.dataset}_pretain_{args.reversed_pretrain}"
